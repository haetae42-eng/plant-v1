...
const PIPELINE_GROUPS = [
  { name: "pots", sourceDir: path.join(ASSETS_ROOT, "pots"), maxDimension: 768 },
  { name: "plants", sourceDir: path.join(ASSETS_ROOT, "plants"), maxDimension: 1024 },
  { name: "sprout", sourceDir: path.join(ASSETS_ROOT, "ui", "sprout"), maxDimension: 768 }
];
...
const toPublicAssetPath = (absPath) => {
...
const optimizeImageAsset = async (sourceAbsPath, maxDimension, toolState) => {
  ...
  const relativeToAssets = path.relative(ASSETS_ROOT, sourceAbsPath);
  ...
  return {
    sourcePath: toPublicAssetPath(sourceAbsPath),
    outputPath: toPublicAssetPath(preferredOutputAbsPath)
  };
};
...
const buildPlantImageMap = (speciesMeta, candidateFiles, sourceToOptimized) => {
  const candidates = candidateFiles.map((absPath) => {
    ...
    const publicPath = toPublicAssetPath(absPath);
    return {
      publicPath,
