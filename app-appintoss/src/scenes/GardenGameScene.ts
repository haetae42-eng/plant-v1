...
const ATTENDANCE_REWARD_CARD_IMAGE_PATH = "";
...
const HARVEST_NICKNAME_INPUT_CENTER_Y = 458;
const HARVEST_NICKNAME_INPUT_BASE_WIDTH = 214;
const HARVEST_NICKNAME_INPUT_BASE_HEIGHT = 34;
const STARTER_SEED_VARIETY_TARGET = 5;
const STARTER_SEED_VARIETY_PRIORITY_IDS = [
  "plant_tulip",
  "plant_rose",
  "plant_daisy",
  "plant_monstera",
...
const SEED_DROP_SOURCE_TEXTURE_KEY = "ui_seed_drop_source";
const SEED_DROP_CUTOUT_TEXTURE_KEY = "ui_seed_drop_cutout";
const SEED_DROP_IMAGE_PATH = "assets/ui/ui_seed_drop.png";
const SEED_DROP_START_OFFSET_Y = 100;
const SEED_DROP_MAX_DISPLAY_WIDTH = 34;
const SEED_DROP_MAX_DISPLAY_HEIGHT = 34;
...
function getDecorFlowerTextureKey(speciesId: string): string {
  // Reuse collection texture keys to avoid loading the same source image twice.
  return getCollectionFlowerTextureKey(speciesId);
}

function getDecorFlowerCutoutTextureKey(speciesId: string): string {
  return getCollectionFlowerCutoutTextureKey(speciesId);
}
...
export class GardenGameScene extends Phaser.Scene {
  ...
  private deferredVisualAssetsReady = false;
  private deferredVisualAssetsReadyCallbacks: Array<() => void> = [];
  private pendingBackgroundTextureKeys = new Set<string>();
  ...
  preload(): void {
    ...
    queueCriticalImage(SEED_DROP_SOURCE_TEXTURE_KEY, SEED_DROP_IMAGE_PATH);
  ...
  private onDeferredVisualAssetsReady(callback?: () => void): void {
    if (!callback) {
      return;
    }
    if (this.deferredVisualAssetsReady) {
      callback();
      return;
    }
    this.deferredVisualAssetsReadyCallbacks.push(callback);
  }

  private notifyDeferredVisualAssetsReady(): void {
    this.deferredVisualAssetsReady = true;
    const callbacks = this.deferredVisualAssetsReadyCallbacks.splice(0, this.deferredVisualAssetsReadyCallbacks.length);
    callbacks.forEach((callback) => {
      try {
        callback();
      } catch (error) {
        console.error("[loader] deferred ready callback failed", error);
      }
    });
  }

  private queueDeferredVisualAssets(onReady?: () => void): void {
    this.onDeferredVisualAssetsReady(onReady);
    ...
    queueDeferredImage(SEED_DROP_SOURCE_TEXTURE_KEY, SEED_DROP_IMAGE_PATH);
    ...
    if (ATTENDANCE_REWARD_CARD_IMAGE_PATH && hasOptimizedSourceAsset(ATTENDANCE_REWARD_CARD_IMAGE_PATH)) {
    ...
    if (queuedCount <= 0) {
      ...
      this.notifyDeferredVisualAssetsReady();
      return;
    }
  ...
  private showStartupSuspenseLoading(): void {
    if (this.startupSuspenseLayer) {
      return;
    }
    const layer = this.add.container(0, 0).setDepth(120);
    this.startupSuspenseLayer = layer;

    layer.add(this.add.rectangle(195, 422, 390, 844, 0x172118, 0.84));
    layer.add(this.add.rectangle(195, 422, 300, 184, 0xf7f0dd, 0.98).setStrokeStyle(2, 0xbaa887, 0.96));
    layer.add(this.add.circle(195, 368, 22, 0x5f8a54, 0.16).setStrokeStyle(2, 0x5f8a54, 0.66));
    layer.add(this.add.circle(195, 368, 8, 0x5f8a54, 0.84));
    const title = this.add
      .text(195, 420, "정원을 준비하고 있어요", {
  ...
  private hideStartupSuspenseLoading(): void {
    if (!this.startupSuspenseLayer) {
      return;
    }
  ...
  create(): void {
    ...
    this.renderAll();
    this.showStartupSuspenseLoading();
    this.queueDeferredVisualAssets(() => {
      this.hideStartupSuspenseLoading();
    });
    this.saveRepository.saveDebounced(this.saveData);
  ...
  private createSeedDropTexture(): void {
    ...
    if (this.textures.exists(SEED_DROP_SOURCE_TEXTURE_KEY) && !this.textures.exists(SEED_DROP_CUTOUT_TEXTURE_KEY)) {
      this.createCutoutTextureFromLoadedImage(SEED_DROP_SOURCE_TEXTURE_KEY, SEED_DROP_CUTOUT_TEXTURE_KEY, 0.08);
    }
    createFallbackSeedTexture(SEED_DROP_TEXTURE_KEY);
  }

  private getSeedDropTextureKey(): string | null {
    if (this.textures.exists(SEED_DROP_CUTOUT_TEXTURE_KEY)) {
      return SEED_DROP_CUTOUT_TEXTURE_KEY;
    }
    if (this.textures.exists(SEED_DROP_SOURCE_TEXTURE_KEY)) {
      return SEED_DROP_SOURCE_TEXTURE_KEY;
    }
    if (this.textures.exists(SEED_DROP_TEXTURE_KEY)) {
      return SEED_DROP_TEXTURE_KEY;
    }
    return null;
  }
  ...
  private createCollectionFlowerCutoutTextures(): void {
    // Disable bulk cutout generation on startup to reduce mobile memory pressure.
    // UI components already fallback to source texture if cutout is absent.
  }

  private createDecorFlowerCutoutTextures(): void {
    // Decor and collection share texture keys to reduce duplicate GPU memory.
    // Collection cutout generation is sufficient for both tabs.
  }

  private createDecorPixelSampleCutoutTextures(): void {
    // Disable upfront cutout generation for pixel sample preview textures.
  }

  private createPotPreviewCutoutTextures(): void {
    // Disable bulk pot cutout generation on startup to reduce memory usage.
  }
  ...
  private buildShell(): void {
    ...
    const seedValueDisplay = createResourceValueWithIcon(seedX, [this.getSeedDropTextureKey() ?? SEED_ICON_TEXTURE_KEY]);
  ...
  private renderGardenBackground(panel: Phaser.GameObjects.Container, backgroundId: string): void {
    ...
    // Load only the backgrounds we currently need instead of preloading the whole catalog.
    this.ensureBackgroundTextureLoaded(selectedBackground.id);
    this.ensureBackgroundTextureLoaded(HOME_DEFAULT_VIEW_BACKGROUND_ID);
  ...
  private ensureBackgroundTextureLoaded(backgroundId: string): void {
    this.ensureBackgroundTexturesLoaded([backgroundId]);
  }

  private ensureBackgroundTexturesLoaded(backgroundIds: readonly string[]): void {
    let queuedCount = 0;
    for (const backgroundId of backgroundIds) {
      const config = BACKGROUND_IMAGE_CONFIG[backgroundId];
      if (!config) {
        continue;
      }
      if (this.textures.exists(config.textureKey) || this.pendingBackgroundTextureKeys.has(config.textureKey)) {
        continue;
      }
      this.pendingBackgroundTextureKeys.add(config.textureKey);
      this.load.image(config.textureKey, resolveAssetPath(config.assetPath));
      queuedCount += 1;
    }
    ...
    const handleLoadError = (file: Phaser.Loader.File): void => {
      if (!this.pendingBackgroundTextureKeys.has(file.key)) {
        return;
      }
      this.pendingBackgroundTextureKeys.delete(file.key);
      console.warn(`[loader] background load failed: ${file.key} (${file.src ?? "unknown src"})`);
    };
    ...
    this.load.once(Phaser.Loader.Events.COMPLETE, () => {
      ...
      for (const key of Array.from(this.pendingBackgroundTextureKeys)) {
        if (this.textures.exists(key)) {
          this.pendingBackgroundTextureKeys.delete(key);
        }
      }
      this.renderMain();
    });
  ...
  private renderShopPurchaseItems(panel: Phaser.GameObjects.Container): void {
    ...
    const seedIconKey = this.getSeedDropTextureKey() ?? SEED_ICON_TEXTURE_KEY;
  ...
  private renderHomeAttendanceOverlay(panel: Phaser.GameObjects.Container): void {
    ...
    const claimedDayCount = Phaser.Math.Clamp(Math.floor(this.saveData.attendance.cycleDay), 0, 7);
    const seedIconTextureKey = this.getSeedDropTextureKey() ?? SEED_ICON_TEXTURE_KEY;
    const attendanceGemIconKey = this.getAttendanceGemIconTextureKey();
  ...
  private renderBackgroundShopItems(panel: Phaser.GameObjects.Container): void {
    ...
    this.ensureBackgroundTexturesLoaded(pageItems.map((background) => background.id));
  ...
  private playSeedDropAnimation(): void {
    ...
    const selectedSeedTextureKey = this.getSeedDropTextureKey();

    let seed: Phaser.GameObjects.Image | Phaser.GameObjects.Ellipse;
    if (selectedSeedTextureKey) {
      const seedImage = this.add.image(seedStartX, seedStartY, selectedSeedTextureKey);
      const sourceWidth = Math.max(1, Math.floor(seedImage.width));
      const sourceHeight = Math.max(1, Math.floor(seedImage.height));
      const scale = Math.min(SEED_DROP_MAX_DISPLAY_WIDTH / sourceWidth, SEED_DROP_MAX_DISPLAY_HEIGHT / sourceHeight);
      seedImage.setDisplaySize(
        Math.max(1, Math.round(sourceWidth * scale)),
        Math.max(1, Math.round(sourceHeight * scale))
      );
      seed = seedImage;
    } else {
      seed = this.add.ellipse(seedStartX, seedStartY, 14, 18, 0xe9be72).setStrokeStyle(2, 0xa66f30);
    }
  ...
  private createHarvestNicknameInputElement(initialValue: string): HTMLInputElement | null {
    ...
    if (window.getComputedStyle(appRoot).position === "static") {
    ...
    input.maxLength = MAX_PLANT_NICKNAME_LENGTH;
    ...
    appRoot.querySelectorAll<HTMLInputElement>("input[data-harvest-nickname-input='true']").forEach((staleInput) => {
      staleInput.remove();
    });
    input.dataset.harvestNicknameInput = "true";
  ...
  private layoutHarvestNicknameInputElement(input: HTMLInputElement): void {
    ...
    const inputRoot = input.offsetParent instanceof HTMLElement ? input.offsetParent : input.parentElement;
    if (!(inputRoot instanceof HTMLElement)) {
      return;
    }

    // Use offset coordinates inside the same positioned root to avoid iOS visualViewport jitter
    // that can happen when converting from global getBoundingClientRect() values.
    const canvasWidth = Math.max(1, canvas.clientWidth);
    const canvasHeight = Math.max(1, canvas.clientHeight);
    const centerX = canvas.offsetLeft + (HARVEST_NICKNAME_INPUT_CENTER_X / 390) * canvasWidth;
    const centerY = canvas.offsetTop + (HARVEST_NICKNAME_INPUT_CENTER_Y / 844) * canvasHeight;
    const width = (HARVEST_NICKNAME_INPUT_BASE_WIDTH / 390) * canvasWidth;
    const height = (HARVEST_NICKNAME_INPUT_BASE_HEIGHT / 844) * canvasHeight;
    input.style.left = `${Math.round(centerX)}px`;
    input.style.top = `${Math.round(centerY)}px`;
  ...
  private applyStarterSeedVarietyMigration(data: SaveDataV1): void {
    const totalSeeds = getTotalSeedCount(data.inventory.seedCounts);
    if (totalSeeds <= 0 || totalSeeds > 12) {
      return;
    }
    if (data.collection.discoveredSpeciesIds.length > 0) {
      return;
    }
    if (data.garden.slots.some((slot) => slot.planted)) {
      return;
    }
    const hasCollectedFlower = AVAILABLE_PLANT_SPECIES_DEFS.some(
      (species) =>
        Math.max(0, Math.floor(data.collection.flowerCounts[species.id] ?? 0)) > 0 ||
        Math.max(0, Math.floor(data.decor.displayFlowerCounts[species.id] ?? 0)) > 0
    );
    if (hasCollectedFlower) {
      return;
    }

    const positiveSeedSpeciesIds = AVAILABLE_PLANT_SPECIES_DEFS.filter(
      (species) => Math.max(0, Math.floor(data.inventory.seedCounts[species.id] ?? 0)) > 0
    ).map((species) => species.id);
    if (positiveSeedSpeciesIds.length >= 4) {
      return;
    }
    if (!positiveSeedSpeciesIds.every((speciesId) => speciesId === "plant_tulip" || speciesId === "plant_rose")) {
      return;
    }

    const candidateSpeciesIds = [
      ...STARTER_SEED_VARIETY_PRIORITY_IDS,
      ...AVAILABLE_PLANT_SPECIES_DEFS.map((species) => species.id)
    ].filter((speciesId, index, list) => list.indexOf(speciesId) === index && AVAILABLE_PLANT_SPECIES_ID_SET.has(speciesId));
    const distributionTarget = Math.max(
      1,
      Math.min(candidateSpeciesIds.length, Math.min(STARTER_SEED_VARIETY_TARGET, totalSeeds))
    );
    const selectedSpeciesIds = candidateSpeciesIds.slice(0, distributionTarget);
    if (selectedSpeciesIds.length < 3) {
      return;
    }

    for (const species of AVAILABLE_PLANT_SPECIES_DEFS) {
      data.inventory.seedCounts[species.id] = 0;
    }
    selectedSpeciesIds.forEach((speciesId) => {
      data.inventory.seedCounts[speciesId] = 1;
    });

    let remaining = totalSeeds - selectedSpeciesIds.length;
    let cursor = 0;
    while (remaining > 0) {
      const speciesId = selectedSpeciesIds[cursor % selectedSpeciesIds.length];
      data.inventory.seedCounts[speciesId] += 1;
      remaining -= 1;
      cursor += 1;
    }
  ...
  private sanitizeRuntimeData(data: SaveDataV1): void {
    ...
    this.applyStarterSeedVarietyMigration(data);
